...
>
  ...
  <div class="body" bind:this={bodyEl}>
    <div class="meta vt-partlabel">
      {#if SPECTRUM_LABELS[section.spectrum]}
        <span class="theme">{SPECTRUM_LABELS[section.spectrum]}</span>
      {/if}
    ...
    <Giscus term={section.num} {mode} {...GISCUS} />
  </div>
    ...
    <div class="nav">
      {#if prevSection}
        <a href="{base}/{prevSection.num}" class="nav-link">
          ...
          <span class="nav-meta">
            <span class="nav-num">{prevSection.num}{SPECTRUM_LABELS[prevSection.spectrum] ? ' · ' + SPECTRUM_LABELS[prevSection.spectrum] : ''}</span>
      ...
      {#if nextSection}
        <a href="{base}/{nextSection.num}" class="nav-link right">
          <span class="nav-meta">
            <span class="nav-num">{nextSection.num}{SPECTRUM_LABELS[nextSection.spectrum] ? ' · ' + SPECTRUM_LABELS[nextSection.spectrum] : ''}</span>
...
<style>
  ...
  .meta { font-family: var(--sans); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.24em; color: var(--muted); border-top: 1px solid var(--rule); padding-top: 1rem; display: flex; flex-direction: column; gap: 0.6rem; }
  .theme { color: var(--accent); font-weight: 500; }
  .year { font-family: var(--serif); font-style: italic; font-weight: 300; font-size: 1.6rem; text-transform: none; letter-spacing: -0.01em; color: var(--ink); margin-top: 0.2rem; }
  ...
  .source-link { font-family: var(--sans); font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.22em; color: var(--accent); border-bottom: 1px solid transparent; white-space: nowrap; transition: border-color 180ms ease; }
  .source-link:hover { border-color: var(--accent); }

  .eli5 { grid-column: 2; margin-top: 2rem; max-width: 56ch; padding: 1.2rem 1.3rem; border-left: 2px solid var(--accent); }
  .page[data-mode='light'] .eli5 { background: rgba(20, 17, 13, 0.04); }
